...
const EditResumeModal = ({ open, onOpenChange, resume }) => {
    ...
    // ===============================
    // Populate form when resume changes
    // ===============================
    ...
    const onSubmit = async (values) => {
        ...
        const result = await dispatch(
            updateResume({ id: resume.id, data: values })
        );

        if (updateResume.fulfilled.match(result)) {
            toast.success(
                result.payload?.message ?? "Resume updated successfully."
            );
            handleClose();
            return;
        }
        ...
        toast.error(
            result.payload?.message ?? "Unable to update resume."
        );
    };

    // ===============================
    // Ref merge for title input
    // ===============================
        ...
        >
            ...
            >
                <motion.div
                    ...
                    className="relative"
                >
                    {/* ✅ Single <form> wraps body + footer so type="submit" works */}
                    <form onSubmit={handleSubmit(onSubmit)}>

                        {/* Header */}
                        <DialogHeader className="border-b border-zinc-800 p-6">
                            <div className="flex items-start gap-4">
                                <div className="flex h-12 w-12 items-center justify-center border border-zinc-800 bg-zinc-900">
                                    <PencilLine size={22} className="text-red-500" />
                                </div>
                        ...
                        <div className="space-y-6 p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    {...titleRegister}
                                    ref={(el) => {
                                        registerRef(el);           // ✅ give ref to react-hook-form
                                        titleInputRef.current = el; // ✅ keep local ref for focus
                                    }}
                                    disabled={loading}
                                    ...
                                    className="
                                        w-full border border-zinc-800 bg-zinc-900
                                        px-4 py-3 text-white placeholder:text-zinc-500
                                        outline-none transition-all focus:border-red-500
                                    "
                                />

                                {errors.title && (
                                    <p id="resume-title-error" className="text-sm text-red-500">
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    className="
                                        ...
                                        transition-all focus:border-red-500
                                    "
                                >
                        ...
                        <div className="flex flex-col-reverse gap-3 border-t border-zinc-800 p-6 sm:flex-row sm:justify-end">
                            <Button
                                ...
                                onClick={handleClose}        // ✅ resets form + clears errors
                                disabled={loading}
                            >
                                Cancel
                            </Button>
                            ...
                            <Button
                                type="submit"               // ✅ now inside form, works correctly
                                disabled={loading}
                    ...
                    {/* Loading Overlay — outside form but inside motion.div with relative */}
                    {loading && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                            <div className="flex items-center gap-3 border border-zinc-800 bg-zinc-950 px-6 py-4">
                                <Loader2 className="h-5 w-5 animate-spin text-red-500" />
                                <span className="text-sm text-zinc-300">Saving changes...</span>
                            </div>
                        </div>
                    )}
